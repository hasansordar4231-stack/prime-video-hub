...
// সরাসরি JSON ফাইল থেকে ডাটা ইমপোর্ট করা হচ্ছে
import categoriesData from '@/data/categories.json';
import videosData from '@/data/videos.json';

export default function Home() {
  // ফিচারড ভিডিও বের করা (is_featured: 1 থাকলে)
  const featured = videosData.filter(v => v.is_featured === 1);
  
  // ভিডিও আছে এমন ক্যাটাগরিগুলো বের করা
  const categoriesWithVideos = categoriesData.map(cat => {
      const videos = videosData.filter(v => v.category_id === cat.id).slice(0, 10);
      return { ...cat, videos };
  }).filter(cat => cat.videos.length > 0); 

  return (
    <div className="pb-10 bg-gray-950 min-h-screen text-white">
      {/* বড় ব্যানার বা ফিচারড সেকশন */}
      {featured.length > 0 && (
